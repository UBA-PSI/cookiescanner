...
#from abp.filters.parser import filters
...
class CookiebannerScanModule(ScanModule):
    ...
    def scan_site(self, result, meta):
        ...
        with ChromeBrowser(debugging_port) as browser:
            ...
            tab.Network.enable()
            tab.Page.enable()
            tab.Page.navigate(url=result['site_url'], _timeout=15)
            tab.wait(3)
            result['frame_resource_tree'] = tab.Page.getResourceTree()
            tab.stop()
...
class ChromeBrowser:
    def __init__(self, abp_filter_names, debugging_port=9222, chrome_executable=None):
...
class WebpageScanner:
    ...
    def do_click(self, click):
        ...
        if len(cookie_notices) > click.cookie_notice_index:
            ...
            if len(clickables) > click.clickable_index:
                ...
                #self.tab.Page.navigateToHistoryEntry(self.tab.Page.getNavigationHistory()["currentIndex"])
                ...
                if self._does_node_exist(cookie_notice.get('node_id')):
                    ...
                    #self.tab.Page.navigateToHistoryEntry(self.tab.Page.getNavigationHistory()["currentIndex"] - 1)
                    #print(self.tab.Page.getNavigationHistory()["currentIndex"])
    ...
    def detect_cookie_notices(self, take_screenshots=True):
        ...
        # find full-width parent nodes with string `cookie`
        cookie_notice_opacity = self.find_cookie_notices_by_opacity(cookie_node_ids)
        cookie_notice_opacity = self._filter_visible_nodes(cookie_notice_opacity)
        self.result.add_cookie_notices('opacity', self.get_properties_of_cookie_notices(cookie_notice_opacity))
        ...
        if take_screenshots:
            ...
            self.take_screenshots_of_visible_nodes(cookie_notice_opacity, 'opacity')
    ...
    def _get_properties_of_cookie_notice(self, node_id):
        ...
        try:
            modified_clickables = self.find_clickables_in_node_modified(node_id)
            modified_clickables_properties = self.get_properties_of_clickables(modified_clickables)
            print(modified_clickables)
            print(modified_clickables_properties)
            clickables = self.find_clickables_in_node(node_id)
            clickables_properties = self.get_properties_of_clickables(clickables)

            print(clickables)
            print(clickables_properties)
    ...
    def _find_fixed_parent(self, node_id):
        js_function = """
            function findFixedParent(elem) {
                ...
                while(elem && elem.parentNode !== body) {
    ...
    ############################################################################
    # COOKIE NOTICE DETECTION: OPACITY
    ############################################################################

    def find_cookie_notices_by_opacity(self, cookie_node_ids):
        cookie_notice_fixed_node_ids = set()
        for node_id in cookie_node_ids:
            fp_result = self._find_by_opacity(node_id)
            if fp_result.get('has_opacity'):
                cookie_notice_fixed_node_ids.add(fp_result.get('opacity'))
        return cookie_notice_fixed_node_ids

    def _find_by_opacity(self, node_id):
        js_function = """
            function findByOpacity(elem) {
                if (!elem) elem = this;
                while(elem && elem.parentNode !== document) {
                    let style = getComputedStyle(elem);
                    if (style.opacity == 1 && elem.parentNode.tagName == "BODY") {
                        return elem;
                    }
                    elem = elem.parentNode;
                }
        ...
        try:
            remote_object_id = self._get_remote_object_id_by_node_id(node_id)
            result = self.tab.Runtime.callFunctionOn(functionDeclaration=js_function, objectId=remote_object_id, silent=False).get('result')
            result_node_id = self._get_node_id_for_remote_object(result.get('objectId'))
            ...
            if self._is_html_node(result_node_id):
                ...
                # otherwise, the frame is considered to be the fixed parent
                    ...
                    return {
                        'has_opacity': True,
                        'opacity': frame_node_id,
                    }
            # otherwise, the returned parent element is a fixed element
            else:
                return {
                    'has_opacity': True,
                    'opacity': result_node_id,
                }
        except pychrome.exceptions.CallMethodException as e:
            self.result.add_warning({
                'message': str(e),
    ...
    def find_clickables_in_node_modified(self, node_id):
        # getEventListeners()
            ...
            function findClickablesInElement(elem) {
                function findCoveringNodes(nodes) {
                    ...
                    for(let arr of nodes){
                        covering_nodes = covering_nodes.concat(...arr)
                    }
                    ...
                    for (var i = 0; i < nodes.length; i++) {
                        ...
                        for (var j = 0; j < nodes.length; j++) {
                            ...
                            // if (node1 !== node2 && node1.includes(node2)){
                ...
                if (!elem) elem = this;
                var nodes = []
                nodes.push(elem.querySelectorAll('a, button, input[type="button"], input[type="submit"], [role="button"], [role="link"]'))
                elem.querySelectorAll('iframe').forEach(
                item => nodes.push(item.contentWindow.document.body.querySelectorAll('a, button, input[type="button"], input[type="submit"], [role="button"], [role="link"]'))
                )
    ...
    def _check_if_node_exists(self, node_id):
        js_function = """
            function clickNode(elem) {
                if (!elem) elem = this;
